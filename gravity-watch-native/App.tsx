...
export default function App() {
  ...
  const handleDownload = async (video: any) => {
    try {
      // Backend'den direkt indirme linkini iste
      const response = await youtubeService.getDownloadUrl(video.id);
      if (response && response.download_url) {
        Alert.alert(
          'İndirme Başlatılıyor',
          `${video.title} indirilsin mi?`,
          [
            { text: 'İptal', style: 'cancel' },
            { 
              text: 'İndir', 
              onPress: () => Linking.openURL(response.download_url) 
            },
          ]
        );
      } else {
        Alert.alert('Hata', 'İndirme bağlantısı alınamadı.');
      }
    } catch (err) {
