...
# --- JARVIS PRIME (v4.7 - 62 SCORE VERSION) ---
...
class JarvisEye(ctk.CTk):
    def __init__(self):
        ...
        self.title("J.A.R.V.I.S. Eye - PRIME 62")
        ...
        self.status = ctk.CTkEntry(self.cp, placeholder_text="Prime Sistemler Hazır...", width=360)
        ...
        with mss.mss() as sct:
            while self.is_vision_active:
                try:
                    ...
                    # Renk Maskeleri (Daha seçici sarı)
                    ...
                    if cnts_p and bird_x:
                        ...
                        if pipes:
                            ...
                            if len(current) >= 2:
                                ...
                                target_y = (top_pipe_bottom + current[1][1]) // 2
                            elif len(current) == 1:
                                p = current[0]
                                if p[1] > h // 2: # Sadece alt boru
                                    target_y = p[1] - (self.GAP_SIZE // 2)
                                    top_pipe_bottom = target_y - (self.GAP_SIZE // 2)
                                else: # Sadece üst boru
                                    top_pipe_bottom = p[1] + p[3]
                                    target_y = top_pipe_bottom + (self.GAP_SIZE // 2)

                    # --- PREDICTIVE REFLEKS MOTORU ---
                    if self.is_auto_mode and bird_y:
                        velocity = bird_y - self.last_bird_y
                        predicted_y = bird_y + (velocity * 2) 
                        on_ground = bird_y > h - 60
                        safe_from_ceiling = (predicted_y - top_pipe_bottom) > 62
                        status = f"B:{bird_y}|P:{int(predicted_y)}|T:{top_pipe_bottom} "
                        
                        if on_ground:
                            self.log(status + "BEKLEMEDE")
                        elif predicted_y > target_y + 12 and safe_from_ceiling:
                    ...
                    if self.debug.get():
                        ...
                        cv2.imshow("Jarvis Prime 62", frame)
                        cv2.waitKey(1)
