...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Download button – uses current quality selection
    downloadBtn.onclick = async () => {
        if (!currentVideo) return;
        downloadStatus.style.display = 'block';
        downloadMsg.textContent = "İndirme başlatıldı...";

        // Determine selected quality (e.g., 1080p, 720p, 480p, 360p)
        const qualityText = qualitySelector.options[qualitySelector.selectedIndex].text;
        let requestedHeight = '1080';
        if (qualityText.includes('720')) requestedHeight = '720';
        else if (qualityText.includes('480')) requestedHeight = '480';
        else if (qualityText.includes('360')) requestedHeight = '360';
