...
# CORS Ayarları: GitHub Pages ve Mobil uygulama için genişletildi
...
def stream_download():
    video_id = request.args.get('video_id')
    requested_quality = request.args.get('quality', '1080')
    if not video_id: return "Missing video_id", 400
    
    from resolver import get_video_info
    info = get_video_info(video_id)
    
    if info.get('error'):
        return info['error'], 500
        
    # Prefer progressive URL for easier streaming/saving on mobile
    download_url = info.get('best_url')
    
    if not download_url:
        return "Download URL not found", 404
        
    # Return a redirect or stream. Redirect is better if the URL is accessible.
    # However, proxying ensures it works everywhere.
    try:
...
def download_video():
    ...
    # Extract requested quality (default 1080)
    requested_quality = data.get('quality', '1080')
    try:
        requested_quality = int(requested_quality)
    except (ValueError, TypeError):
        requested_quality = 1080
    # Build yt-dlp format filter
    quality_filter = f"height<={requested_quality}"
    print(f"[DEBUG] Requested quality: {requested_quality}")
    print(f"[DEBUG] Using format filter: {quality_filter}")
    ...
    def do_download():
        try:
            from resolver import COOKIES_FILE
            
            # GÜÇLÜ İNDİRME MOTORU v3: "Asla Durdurulamaz" Modu
            ydl_opts = {
                'format': f'bestvideo[ext=mp4][{quality_filter}]+bestaudio[ext=m4a]/best[ext=mp4][{quality_filter}]',
                'outtmpl': os.path.join(download_path, '%(title)s.%(ext)s'),
                'ffmpeg_location': FFMPEG_EXE,
                'merge_output_format': 'mp4',
                'nopart': True, # .PART dosyası oluşturma, direkt yaz
                ...
                'extractor_args': {
                    'youtube': {
                        'player_client': ['tv', 'web_creator', 'android', 'ios'],
                        'player_skip': ['web'], # Standart web kısıtlamalarını atla
                    }
                },
            }
            ...
            # ULTRA FALLBACK: En düşük ama en güvenli kalite (Tek parça)
            try:
                with yt_dlp.YoutubeDL({
                    'format': 'best',
                    'outtmpl': os.path.join(download_path, '%(title)s_safe.%(ext)s'),
                    'nopart': True,
                    'cookiefile': COOKIES_FILE if os.path.exists(COOKIES_FILE) else None
                }) as ydl:
                    ydl.download([f"https://www.youtube.com/watch?v={video_id}"])
