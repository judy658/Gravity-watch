...
const OBSTACLE_HIT_SIZE = 50; // Hitbox smaller than visual
const OBSTACLE_OFFSET = (OBSTACLE_SIZE - OBSTACLE_HIT_SIZE) / 2; // Center hitbox
...
function killPlayer(victimId, killerId) {
  ...
  if (killFeed.length > 5) killFeed.shift(); // Son 5 bildirimi tut
        ...
        io.emit('killFeed', feedItem); // Broadcast for clients
